"""data/knowledge_base/generated_admin_team_dataset.json(행정팀 담당 신청/등록 FAQ,
신청·등록·발급·서류·접수 키워드 커버)을 Supabase(admin_team_faq, 답변 저장) + Chroma
(chroma_merged_faq, 질문만 임베딩)로 적재한다. 신청_행정_분류 규칙(rules.yaml)의
data_source 공백을 메운다.
...
실행:
    cd backend
    python -m app.rag.ingest_admin_team_faq load-supabase
    python -m app.rag.ingest_admin_team_faq load-supabase --dry-run
    python -m app.rag.ingest_admin_team_faq load-chroma
    python -m app.rag.ingest_admin_team_faq test
"""
import json
import sys
...
def ingest_supabase(dry_run: bool = False) -> int:
    """generated_admin_team_dataset.json을 읽어 Supabase(admin_team_faq)에만 저장한다."""
...
def test_search() -> None:
    """테스트: '휴학 신청은 어떻게 하나요' 질문으로 검색해 결과를 출력한다."""
