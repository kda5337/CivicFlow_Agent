"""data/knowledge_base/generated_safety_team_dataset.json(안전관리팀 관련 안전수칙/응급처치
+ 사고 신고·조사·보상 절차 FAQ, 사고·부상·화재·위험·안전 키워드 커버)을 Supabase
(safety_team_faq, 답변 저장) + Chroma(chroma_merged_faq, 질문만 임베딩)로 적재한다.

주의: 안전_사고_분류 규칙(rules.yaml)은 관리자검토필요: true라 apply_rules 이후
manager_review로 바로 라우팅되어 retrieve_node/generate_node를 타지 않는다. 즉 이 데이터는
안전관리팀으로 분류된 문의 자체에는 쓰이지 않고, chroma_merged_faq를 공유하는 다른
문의유형(예: 오류/장애·불만/신고 중 안전_사고_분류에 안 걸린 나머지)에서 우연히 유사도가
높으면 참고될 수 있는 정도다. retrieve_node가 담당부서 기준으로 컬렉션을 분리하도록
확장되면 그때 실제로 쓰이게 된다.
...
실행:
    cd backend
    python -m app.rag.ingest_safety_team_faq load-supabase
    python -m app.rag.ingest_safety_team_faq load-supabase --dry-run
    python -m app.rag.ingest_safety_team_faq load-chroma
    python -m app.rag.ingest_safety_team_faq test
"""
import json
import sys
...
def ingest_supabase(dry_run: bool = False) -> int:
    """generated_safety_team_dataset.json을 읽어 Supabase(safety_team_faq)에만 저장한다."""
...
def test_search() -> None:
    """테스트: '사고를 어떻게 신고하나요' 질문으로 검색해 결과를 출력한다."""
    query = "사고를 어떻게 신고하나요"
    vectorstore = get_vectorstore(MERGED_FAQ_COLLECTION_NAME)
