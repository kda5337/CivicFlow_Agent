"""data/knowledge_base/coupang_faq_personal_info.json(쿠팡 개인정보 FAQ, PDF 3편 ep_1~ep_3)을
Supabase(coupang_faq_personal_info, 답변 저장) + Chroma(chroma_merged_faq, 질문만 임베딩)로
적재한다. 회원관리팀 담당 문의(변경/정정, 오류/장애의 계정 관련)의 RAG 근거로 쓰인다.
...
실행:
    cd backend
    python -m app.rag.ingest_coupang_personal_info_faq load-supabase
    python -m app.rag.ingest_coupang_personal_info_faq load-supabase --dry-run
    python -m app.rag.ingest_coupang_personal_info_faq load-chroma
    python -m app.rag.ingest_coupang_personal_info_faq test
"""
import json
import re
import sys
...
INPUT_PATH = (
    ...
    / "data"
    / "knowledge_base"
    / "coupang_faq_personal_info.json"
)

MERGED_FAQ_COLLECTION_NAME = "chroma_merged_faq"
TABLE_NAME = "coupang_faq_personal_info"
...
def ingest_supabase(dry_run: bool = False) -> int:
    """coupang_faq_personal_info.json을 읽어 Supabase(coupang_faq_personal_info)에만 저장한다."""
...
def test_search() -> None:
    """테스트: '회원 탈퇴는 어떻게 하나요' 질문으로 검색해 결과를 출력한다."""
    query = "회원 탈퇴는 어떻게 하나요"
    vectorstore = get_vectorstore(MERGED_FAQ_COLLECTION_NAME)
