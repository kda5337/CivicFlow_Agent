"""data/knowledge_base/generated_legal_team_dataset.json(법무팀 담당 소송/고발/약관/
법적조치/손해배상 FAQ)을 Supabase(legal_team_faq, 답변 저장) + Chroma(chroma_merged_faq,
질문만 임베딩)로 적재한다. 법무_분류 규칙(rules.yaml)의 data_source 공백을 메운다.

법무_분류는 (안전_사고_분류와 달리) 관리자검토필요가 걸려 있지 않아 apply_rules 이후
그대로 retrieve_node/generate_node를 타므로, 이 데이터는 실제로 법무팀 분류 문의의
RAG 근거로 쓰인다.
...
실행:
    cd backend
    python -m app.rag.ingest_legal_team_faq load-supabase
    python -m app.rag.ingest_legal_team_faq load-supabase --dry-run
    python -m app.rag.ingest_legal_team_faq load-chroma
    python -m app.rag.ingest_legal_team_faq test
"""
import json
import sys
...
def ingest_supabase(dry_run: bool = False) -> int:
    """generated_legal_team_dataset.json을 읽어 Supabase(legal_team_faq)에만 저장한다."""
...
def test_search() -> None:
    """테스트: '손해배상 소송을 제기하려면 어떻게 하나요' 질문으로 검색해 결과를 출력한다."""
    query = "손해배상 소송을 제기하려면 어떻게 하나요"
    vectorstore = get_vectorstore(MERGED_FAQ_COLLECTION_NAME)
